...
export class ClientAccountManagementComponent {
  ...
  protected readonly devices = signal<DeviceRow[]>([
    {
      id: 'd1',
      label: 'Este dispositivo · Windows',
      lastActive: new Date(),
      location: 'Lisboa, PT',
      current: true,
    },
    {
      ...
      lastActive: new Date(Date.now() - 86_400_000 * 4),
      location: 'Porto, PT',
      current: false,
    },
    {
      id: 'd3',
      label: 'Safari · macOS',
      lastActive: new Date(Date.now() - 86_400_000 * 18),
  ...
  protected exportData(): void {
    ...
    window.setTimeout(() => {
      ...
      this.flash('Pedido de exportação registado. Receberá um email quando o ficheiro estiver disponível (mock).', 'ok');
    }, 1400);
  }
  ...
  protected confirmDelete(): void {
    this.deleteModalOpen.set(false);
    this.flash(
      'Demonstração: nenhuma conta foi eliminada. Fluxo sensível só após KYB/KYC e políticas internas.',
      'err',
    );
  }
