...
const PROFILE_EXTRAS_KEY = 'bm-client-profile-extras';
...
export class ClientProfileComponent implements OnInit {
  ...
  protected saveProfile(): void {
    ...
    const fallback = this.auth.user()?.email.split('@')[0]?.trim() || 'Cliente';
...
function readProfileExtras(browser: boolean): Partial<ProfileExtras> | null {
  ...
  try {
    ...
    const newsletter = typeof o['newsletter'] === 'boolean' ? o['newsletter'] : undefined;
    const productTips = typeof o['productTips'] === 'boolean' ? o['productTips'] : undefined;
    ...
    if (newsletter !== undefined) {
      out.newsletter = newsletter;
    }
    if (productTips !== undefined) {
      out.productTips = productTips;
    }
    if (digest !== undefined) {
      out.digest = digest;
    }
